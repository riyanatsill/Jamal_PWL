...
$dmlNoPagination = "SELECT id_tr, username, ign, uid, id_item, payment FROM transaksi WHERE username = '$uname'";
...
$dmlDefault = "SELECT id_tr, ign, uid, tdate FROM transaksi WHERE username = '$uname' LIMIT $position, $pagination";
$queryDefault = mysqli_query($con, $dmlDefault);

if (isset($_GET['searchBtn'])) {
    $search = $_GET['searchBtn'];

    $dmlSearch = "SELECT id_tr, ign, uid, tdate FROM transaksi WHERE ign LIKE '%$search%' AND username = '$uname'";
...
<div class="container">
    ...
    <form method="get">
        <div>
            <input type="search" name="searchBtn" id="form1" placeholder="Searching History"/>
    ...
    <ul style="display:flex; justify-content:center;" class="pagination">
        <?php for ($i = 1; $i <= $pages; $i++) { ?>
            <li class="page-item"><a class="btn btn-dark" href="<?php echo ($i != $page) ? "userhistory.php?page=" . $i : "#" ?>"><?= $i ?></a></li>
        <?php } ?>
    </ul>
</div>
</body>
